...
impl Popup for CallPopup {
    fn draw(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        ...
        .style(self.get_button_style(SelectedButton::Yes));
        f.render_widget(yes_button, button_area[0]);

        let yes_button = Paragraph::new("(N)o")
        .alignment(Alignment::Center)
        .style(self.get_button_style(SelectedButton::No));
        ...
        match e {
            Event::Key(e) => {
                match e.code {
                    ...
                    KeyCode::Enter => {
                        match &mut self.selected_button {

...
impl Tui {
    ...
    pub fn settings_input_list(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        match self.settings_inputs.as_ref() {
            ...
            None => {
                let label = Paragraph::new("No input devices found").style(Style::default().fg(Color::Red));
    ...
    pub fn settings_output_list(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        match self.settings_outputs.as_ref() {
            Some(devices) => {
                let output_list =
                    List::new(devices.iter().map(|d| ListItem::new(d.clone())).collect::<Vec<ListItem>>())
                        .block(
                            Block::default()
            ...
            None => {
                let label = Paragraph::new("No output devices found").style(Style::default().fg(Color::Red));
                f.render_widget(label, area);
            }
        }
    }
    ...
    pub fn settings_kbits_list(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        let kbits_list = List::new(
            CHOOSABLE_KBITS.iter().map(|n| ListItem::new(format!("{} kbit/s", n))).collect::<Vec<ListItem>>(),
        )
        .block(
            Block::default()
                .title("Preferred voice bitrate")
                .borders(Borders::ALL)
    ...
    pub fn contact_list(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        let contact_list = List::new(
            self.peers.iter().map(|p| ListItem::new(p.get_public_key().to_string().clone())).collect::<Vec<ListItem>>(),
        )
        .block(
            Block::default()
                .title("Contacts")
    ...
    pub fn chat_input(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        ...
        if !input_string.is_empty() && self.is_active && self.active_block == ActiveBlock::ChatInput {
            f.set_cursor(area.x + input.get_cursor_pos() as u16 + 1, area.y + 1);
            //FIXME: Add support for multiline
        }
    ...
    pub fn debug_messages(&mut self, f: &mut Frame<CrosstermBackend<Stdout>>, area: Rect) {
        let debug_message_list = List::new(
            self.debug_messages
                .iter()
                .map(|p| {
                    ListItem::new(Text::from(p.to_string())).style(match p.msg_type {

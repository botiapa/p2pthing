...
use p2pthing_common::{
    ...
    ui::{CallStatus, CallStatusHolder, CHOOSABLE_KBITS},
};
...
impl Tui {
            ...
            match msg {
                ...
                InterthreadMessage::PeerDisconnected(p_key) => {
                    ...
                    match self.active_block {
                        ActiveBlock::ChatInput | ActiveBlock::ChatMessages => {
                            self.active_block = ActiveBlock::ContactList
                        }
                        _ => {}
                    }
                }
                InterthreadMessage::Call(public_key) => {
                    ...
                    self.active_popup = Some(Box::new(CallPopup::new(public_key.clone())));
                    match self.calls.iter_mut().find(|c| c.public_key == public_key) {
                        Some(call) => call.status = CallStatus::SentRequest,
                        None => self
                            .calls
                            .push(CallStatusHolder { status: CallStatus::SentRequest, public_key: public_key.clone() }),
                    }
                }
                ...
                InterthreadMessage::DebugMessage(msg, msg_type) => {
                    self.debug_messages.push(DebugMessage { message: msg, time: Utc::now(), msg_type });
                ...
                InterthreadMessage::OnChatMessage(msg) => {
                    ...
                    ui_peer.chat_messages.push(ChatMessageUI::from_chat_message(msg, received, own));
                }
    ...
    fn send_chat_message(&mut self) {
        ...
        if !peer.chat_input.get_string().is_empty() {
            self.cm_s
                .as_ref()
                .unwrap()
                .send(InterthreadMessage::SendChatMessage(
                    peer.get_public_key().clone(),
                    peer.chat_input.get_string(),
                    None,
                ))
                .unwrap();
        ...
        match e {
            Event::Key(e) => match e.code {
                ...
                KeyCode::Char('m') | KeyCode::Char('M')
                    if self.active_block != ActiveBlock::ChatInput
                        || (self.active_block == ActiveBlock::ChatInput && !self.is_active) =>
                {
                        ...
                        || (self.active_block == ActiveBlock::ChatInput && !self.is_active) =>
                {
                    ...
                    self.cm_s
                        .as_ref()
                        .unwrap()
                        .send(InterthreadMessage::AudioChangeDenoiserState(self.denoiser))
                        .unwrap();
                }
                KeyCode::F(x) => match x {
    ...
    fn handle_input_list_event(&mut self, e: Event) {
        match e {
            Event::Key(e) => match e.code {
                ...
                KeyCode::Down if self.is_active => match &self.settings_inputs {
                    Some(inputs) => match self.settings_inputs_state.selected() {
                    ...
                    Some(inputs) => match self.settings_inputs_state.selected() {
                        Some(selected) => {
                            ...
                            self.cm_s
                                .as_ref()
                                .unwrap()
                                .send(InterthreadMessage::AudioChangeInputDevice(d.clone()))
                                .unwrap();
                        }
                        None => {}
                    },
    ...
    fn handle_output_list_event(&mut self, e: Event) {
        match e {
            Event::Key(e) => match e.code {
                KeyCode::Up if self.is_active => match &self.settings_inputs {
                    Some(inputs) => match self.settings_outputs_state.selected() {
                        Some(i) if i > 0 => self.settings_outputs_state.select(Some(i - 1)),
                        _ => self.chat_messages_list_state = Some(inputs.len() - 1),
                    },
                    _ => {}
                },
                KeyCode::Down if self.is_active => match &self.settings_outputs {
                    ...
                    Some(outputs) => match self.settings_outputs_state.selected() {
                        Some(selected) => {
                            ...
                            self.cm_s
                                .as_ref()
                                .unwrap()
                                .send(InterthreadMessage::AudioChangeOutputDevice(d.clone()))
                                .unwrap();
                        }
                        None => {}
                    },
    ...
    fn handle_bitrate_list_event(&mut self, e: Event) {
        match e {
            Event::Key(e) => match e.code {
                ...
                KeyCode::Down if self.is_active => match self.settings_kbits_state.selected() {
                    Some(i) if i < CHOOSABLE_KBITS.len() - 1 => self.settings_kbits_state.select(Some(i + 1)),
                    _ => self.settings_kbits_state.select(Some(0)),
                },
                KeyCode::Enter if self.is_active => match self.settings_kbits_state.selected() {
                    Some(selected) => {
    ...
    fn handle_contact_list_event(&mut self, e: Event) {
        match e {
            Event::Key(e) => match e.code {
                KeyCode::Enter if self.is_active => {
                    ...
                    match self.contact_list_state.selected() {
                        Some(i) => {
                            ...
                            self.calls
                                .push(CallStatusHolder { status: CallStatus::SentRequest, public_key: p.clone() });
                ...
                KeyCode::Up => self.active_block = ActiveBlock::Tabs,
                KeyCode::Right if !self.is_active && self.contact_list_state.selected().is_some() => {
                    self.active_block = ActiveBlock::ChatMessages
                }
                _ => {}
            },
            Event::Mouse(_) => {}
    ...
    fn handle_chat_messages_event(&mut self, e: Event) {
        ...
        match e {
            Event::Key(e) => match e.code {
                KeyCode::Up if self.is_active => match self.chat_messages_list_state {
                    Some(i) if i > 0 => self.chat_messages_list_state = Some(i - 1),
                    None if self.chat_messages_length > 1 => {
                        self.chat_messages_list_state = Some(self.chat_messages_length - 2)
                    }
                    _ => {}
                },
                KeyCode::Down if self.is_active => match self.chat_messages_list_state {
                    ...
                    match self.contact_list_state.selected() {
                        Some(i) => {
                            self.cm_s
                                .as_ref()
                                .unwrap()
                                .send(InterthreadMessage::Call(self.peers.get(i).unwrap().get_public_key().clone()))
                                .unwrap();
                        }
                        None => {}
                    }

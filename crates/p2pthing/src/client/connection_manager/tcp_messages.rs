...
use p2pthing_common::{
    ...
    message_type::{
        msg_types::{self, AnnounceRequest, AnnounceSecret, Call, CallResponse, Disconnect},
...
use crate::client::peer::{Peer, PeerSource, PeerType};
...
impl ConnectionManager {
    ...
    fn on_announce_request(&mut self, addr: SocketAddr, announcement: AnnounceRequest) {
        ...
        let announce_secret = msg_types::AnnounceSecret { secret: conn.symmetric_key.as_ref().unwrap().secret.clone() };
        self.send_tcp_message_public_key(MsgType::AnnounceSecret, &announce_secret).unwrap();

        let announce_public = msg_types::AnnouncePublic { public_key: self.encryption.get_public_key().clone() };
    ...
    fn on_call_response(&mut self, _: SocketAddr, call_response: CallResponse) {
        ...
        if !call_response.response {
            let i = self.calls_in_progress.iter().position(|(c, _)| c.callee == call.callee).unwrap();
            ...
            if let Some(i) = self.calls_in_progress.iter().position(|(c, _)| c.callee == call.callee) {
                ...
                self.ui_s.log_info(&format!(
                    "A sent call has been accepted by peer ({};{}), starting the punch through protocol",
                    call.callee, conn.address
                ));

                conn.send_udp_message_with_public_key(
                    MsgType::AnnounceSecret,
                    &AnnounceSecret { secret: conn.symmetric_key.as_ref().unwrap().secret.clone() },
                    true,
                    None,
                )
                .unwrap();
                ...
                self.ui_s.log_warning(&format!(
                    "An invalid call has been accepted by address ({}), discarding",
                    udp_address
                ));
            }
        }

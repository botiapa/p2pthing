...
impl ReceivableChunk {
    pub fn new() -> ReceivableChunk {
        ReceivableChunk { requested: false, received: false }
    }
}
...
impl FileManager {
    ...
    pub fn start_sending_file(&mut self, filename: String) -> io::Result<PreparedFile> {
        ...
        return Ok(PreparedFile { file_id, file_name: filename.to_string(), file_extension: extension, total_length });
    }
    ...
    pub fn start_receiving_file(&mut self, file: PreparedFile, sender: NetworkedPublicKey) -> io::Result<()> {
        ...
        if !self.receiving_chunks.contains_key(&file.file_id) {
            let x = self
                .receiving_chunks
                .insert(file.file_id.clone(), vec![ReceivableChunk::new(); chunk_count.try_into().unwrap()]);
            assert!(x.is_none());
        ...
        for file in files {
            if self.is_file_done(&file) {
                ...
                if let Ok(elapsed) = stats.started.elapsed() {
                    ...
                    trace!(
                        "File transfer ({}) done in {}ms achieving {:.} MB/s",
                        &file[0..10],
                        elapsed.as_millis(),
                        mbs
                    );
    ...
    fn open_file(
        ...
        if !self.open_files.contains_key(file_id) {
            ...
            self.open_files.insert(file_id.clone(), OpenFile { file, metadata });

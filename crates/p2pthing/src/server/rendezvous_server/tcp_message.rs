...
use p2pthing_common::{
    ...
    message_type::{
        msg_types::{self, AnnouncePublic, AnnounceSecret, Call, CallResponse},
        MsgType,
    },
    num, read_exact,
};
...
impl RendezvousServer {
    ...
    fn on_announce(&mut self, addr: SocketAddr, announcement: AnnouncePublic) {
        ...
        RendezvousServer::send_tcp_message(
            sock,
            MsgType::Announce,
            &self.peers.to_vec().iter_mut().map(|x| x.public_key.clone()).collect::<Vec<_>>(),
        );
        ...
        if let Some(caller) = self.peers.iter().find(|x| x.addr.unwrap() == addr) {
            if let Some(callee) = self.peers.iter().find(|x| x.public_key == call.callee) {
                if caller.udp_addr.is_none() || callee.udp_addr.is_none() {
                    ...
                    RendezvousServer::send_tcp_message(
                        &mut caller_socket,
                        MsgType::CallResponse,
                        &CallResponse { call: call.clone(), response: false },
                    );
                    println!(
                        "Error routing a call from ({}; {}) to ({}; {}) udp address hasn't been found",
                        addr,
                        caller.public_key,
                        callee.addr.unwrap(),
                        callee.public_key
                    ...
                    let req = CallRequest { caller: caller.clone(), callee: callee.clone() };
                    self.calls.push(req);
                    ...
                    println!(
                        "Routed a call from ({}; {}) to ({}; {})",
                        addr,
                        caller.public_key,
                        callee_socket.peer_addr().unwrap(),
                        callee.public_key
                    );
                }
            } else {
            ...
            Some(index) => {
                if call_response.response {
                    ...
                    let mut sock = self
                        .tcp_connections
                        .values_mut()
                        .find(|x| x.peer_addr().unwrap() == caller_peer.addr.unwrap())
                        .unwrap();

                    let msg = msg_types::CallResponse {
                        call: Call { callee, caller: Some(caller), udp_address: Some(callee_peer.udp_addr.unwrap()) },
